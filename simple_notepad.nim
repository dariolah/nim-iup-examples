...
proc set_find_replace_visibility(find_dlg:PIhandle, show_replace:bool) =
  let
    replace_txt = iup.getDialogChild(find_dlg, "REPLACE_TEXT")
    replace_lbl = iup.getDialogChild(find_dlg, "REPLACE_LABEL")
    replace_bt = iup.getDialogChild(find_dlg, "REPLACE_BUTTON")

  if show_replace:
    iup.setAttribute(replace_txt, "VISIBLE", "Yes")
    iup.setAttribute(replace_lbl, "VISIBLE", "Yes")
    iup.setAttribute(replace_bt, "VISIBLE", "Yes")
    iup.setAttribute(replace_txt, "FLOATING", "No")
    iup.setAttribute(replace_lbl, "FLOATING", "No")
    iup.setAttribute(replace_bt, "FLOATING", "No")

    iup.setAttribute(find_dlg, "TITLE", "Replace")
  else:
    iup.setAttribute(replace_txt, "FLOATING", "Yes")
    iup.setAttribute(replace_lbl, "FLOATING", "Yes")
    iup.setAttribute(replace_bt, "FLOATING", "Yes")
    iup.setAttribute(replace_txt, "VISIBLE", "No")
    iup.setAttribute(replace_lbl, "VISIBLE", "No")
    iup.setAttribute(replace_bt, "VISIBLE", "No")

    iup.setAttribute(find_dlg, "TITLE", "Find")

  iup.setAttribute(find_dlg, "SIZE", nil);  # force a dialog resize on the IupRefresh
...
proc item_save_action_cb(item_save:PIhandle):int =
  ...
  if filename != "":
    ...
    let dirty = iup.getInt(multitext, "DIRTY")
    if dirty == 1:
      discard save_file(multitext)
  return IUP_DEFAULT
...
proc find_next_action_cb(ih:PIhandle): int =
  let find_dlg = cast[PIhandle](iup.getAttribute(ih, "FIND_DIALOG"))
  if find_dlg == nil:
    return IUP_DEFAULT

  let multitext = cast[PIhandle](iup.getAttribute(find_dlg, "MULTITEXT"))
  var find_pos = iup.getInt(multitext, "FIND_POS")

  let txt = iup.getDialogChild(find_dlg, "FIND_TEXT")
  let str_to_find = iup.getAttribute(txt, "VALUE")

  let find_case = iup.getDialogChild(find_dlg, "FIND_CASE")
  ...
  if pos >= 0:
    ...
    iup.setfAttribute(multitext, "FIND_SELECTION", "%d:%d", unicode_pos, unicode_end_pos)
    ...
    iupfix.setInt(multitext, "FIND_POS", -1)
...
proc find_replace_action_cb(bt_replace:PIhandle):int =
  let
    find_dlg = cast[PIhandle](iup.getAttribute(bt_replace, "FIND_DIALOG"))
    multitext = cast[PIhandle](iup.getAttribute(find_dlg, "MULTITEXT"))
    find_pos = iup.getInt(multitext, "FIND_POS")
    selectionpos = iup.getAttribute(multitext, "SELECTIONPOS")
    find_selection = iup.getAttribute(multitext, "FIND_SELECTION")
  ...
  if find_pos == -1 or selectionpos == nil or find_selection == nil or selectionpos != find_selection:
    ...
    let
      replace_txt = iup.getDialogChild(find_dlg, "REPLACE_TEXT")
      str_to_replace = iup.getAttribute(replace_txt, "VALUE")
    iup.setAttribute(multitext, "SELECTEDTEXT", str_to_replace)
...
proc find_close_action_cb(bt_close:PIhandle):int =
  let
    find_dlg = iup.getDialog(bt_close)
    multitext = cast[PIhandle](iup.getAttribute(find_dlg, "MULTITEXT"))
    config = cast[PIhandle](iup.getAttribute(multitext, "CONFIG"))

  iupfix.configDialogClosed(config, find_dlg, "FindDialog")
  iup.hide(find_dlg)  # do not destroy, just hide
...
proc create_find_dialog(multitext:PIhandle):PIhandle =
  ...
  iup.setAttribute(txt, "VISIBLECOLUMNS", "20")
  txt_replace = iup.text(nil)
  iup.setAttribute(txt_replace, "NAME", "REPLACE_TEXT")
  iup.setAttribute(txt_replace, "VISIBLECOLUMNS", "20")
  ...
  bt_replace = iup.button("Replace", nil)
  iup.setAttribute(bt_replace, "PADDING", "10x2")
  iup.setCallback(bt_replace, "ACTION", cast[ICallback](find_replace_action_cb))
  iup.setAttribute(bt_replace, "NAME", "REPLACE_BUTTON")
  ...
  box = iup.vbox(
    iup.label("Find What:"),
    txt,
    iup.setAttributes(iup.label("Replace with:"), "NAME=REPLACE_LABEL"),
    txt_replace,
    find_case,
    iup.setAttributes(iup.hbox(
      iup.fill(),
  ...
  find_dlg = iup.dialog(box)
  iup.setAttribute(find_dlg, "TITLE", "Find")
  iup.setAttribute(find_dlg, "DIALOGFRAME", "Yes")
  iup.setAttributeHandle(find_dlg, "DEFAULTENTER", bt_next)
  iup.setAttributeHandle(find_dlg, "DEFAULTESC", bt_close)
  iup.setAttributeHandle(find_dlg, "PARENTDIALOG", iup.getDialog(multitext))
  iup.setCallback(find_dlg, "CLOSE_CB", cast[ICallback](find_close_action_cb))

  # Save the multiline to access it from the callbacks
  iup.setAttribute(find_dlg, "MULTITEXT", cast[cstring](multitext))

  # Save the dialog to reuse it
  iup.setAttribute(find_dlg, "FIND_DIALOG", cast[cstring](find_dlg))  # from itself
  iup.setAttribute(iup.getDialog(multitext), "FIND_DIALOG", cast[cstring](find_dlg)) # from the main dialog

  return find_dlg

proc item_find_action_cb(item_find:PIhandle): int =
  var find_dlg = cast[PIhandle](iup.getAttribute(item_find, "FIND_DIALOG"))
  let
    multitext = iup.getDialogChild(item_find, "MULTITEXT")
    config = cast[PIhandle](iup.getAttribute(multitext, "CONFIG"))
  ...
  iupfix.configDialogShow(config, find_dlg, "FindDialog")

  let str = iup.getAttribute(multitext, "SELECTEDTEXT")
  if str != nil and str != "":
    let txt = iup.getDialogChild(find_dlg, "FIND_TEXT")
    iupfix.setStrAttribute(txt, "VALUE", str)

  return iup.IUP_DEFAULT

proc item_replace_action_cb(item_replace:PIhandle):int =
  var find_dlg = cast[PIhandle](iup.getAttribute(item_replace, "FIND_DIALOG"))
  let
    multitext = iup.getDialogChild(item_replace, "MULTITEXT")
    config = cast[PIhandle](iup.getAttribute(multitext, "CONFIG"))
...
proc selection_find_next_action_cb(ih:PIhandle):int =
  let multitext = iup.getDialogChild(ih, "MULTITEXT")

  let str = iup.getAttribute(multitext, "SELECTEDTEXT")
  if str != nil and str != "":
    var find_dlg = cast[PIhandle](iup.getAttribute(ih, "FIND_DIALOG"))
...
proc item_help_action_cb():int =
  discard iup.help("http://www.tecgraf.puc-rio.br/iup")
  return IUP_DEFAULT
...
proc create_main_dialog(config:PIhandle):PIhandle =
  var
    ...
    sub_menu_edit, edit_menu, item_find, item_find_next, item_goto: iup.PIhandle
    ...
    sub_menu_format, format_menu, item_font, item_replace: iup.PIhandle
    sub_menu_help, help_menu, item_help, item_about: iup.PIhandle
  ...
  item_find_next = iup.item("Find &Next\tF3", nil)
  iup.setAttribute(item_find_next, "NAME", "ITEM_FINDNEXT")
  iup.setCallback(item_find_next, "ACTION", cast[Icallback](find_next_action_cb))

  item_replace = iup.item("&Replace...\tCtrl+H", nil)
  iup.setCallback(item_replace, "ACTION", cast[Icallback](item_replace_action_cb))
  ...
  item_help= iup.item("&Help...", nil)
  iup.setCallback(item_help, "ACTION", cast[Icallback](item_help_action_cb))
  ...
  view_menu = iup.menu(
    ...
    item_statusbar,
    nil)
  help_menu = iup.menu(item_help, item_about, nil)
  ...
  iup.setCallback(dlg, "K_cN", cast[ICallback](item_new_action_cb))
  ...
  iup.setCallback(dlg, "K_cH", cast[ICallback](item_replace_action_cb)) # replace system processing
  iup.setCallback(dlg, "K_cG", cast[ICallback](item_goto_action_cb))
  iup.setCallback(dlg, "K_F3", cast[ICallback](find_next_action_cb))
  iup.setCallback(dlg, "K_cF3", cast[ICallback](selection_find_next_action_cb))
  iup.setCallback(dlg, "K_cV", cast[ICallback](item_paste_action_cb))
