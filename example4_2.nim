...
# https://www.tecgraf.puc-rio.br/iup/examples/tutorial/example4_2.c
...
proc read_file(filename:string):ptr imImage =
  ...
  if error != IM_ERR_NONE:
    ...
    discard imImageGetOpenGLData(image, nil)
...
proc canvas_action_cb(canvas:PIhandle):cint =
  ...
  glLoadIdentity()
  glOrtho(0, canvas_width.GLdouble, 0, canvas_height.GLdouble, -1, 1)
  ...
  glClearColor(float(ri) / 255.float, float(gi) / 255.float, float(bi) / 255.float, 1)
  ...
  if image != nil:
    ...
    let gldata = imImageGetAttribute(image, "GLDATA", nil, nil)
